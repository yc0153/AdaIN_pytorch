...
- `Content`
  - 스타일을 입힐 게임 타일 이미지
  - `Kenney`에서 수집한 타일을 사용
- `Style`
  - 계절별로 `2D 이미지 5장`, `실사 이미지 5장`씩 수집
  - `Pinterest`에서 봄, 여름, 가을, 겨울 자료를 각각 수집

즉, 각 계절마다 아래처럼 style 그룹을 구성했다.
...
### 왜 이런 방식으로 style을 구성했는가

우리의 목적은 `스타일 이미지 자체를 타일 형식으로 맞추는 것`이 아니라, `계절 스타일만 가지고 기존 게임 타일 전체에 일관된 분위기를 입히는 것`이다.

그래서 style 이미지도 굳이 타일 형식으로 맞춘 데이터를 넣기보다,

- 계절감을 잘 드러내는 2D 이미지
- 계절감을 잘 드러내는 실사 이미지

를 각각 5장씩 모아서 interpolation 하는 방식을 선택했다.

이 방식으로 계절마다 최종적으로 두 종류의 스타일 그룹을 만들었다.

- `계절별 2D 스타일 1개`
- `계절별 실사 스타일 1개`

그리고 이 두 스타일을 모든 개별 타일에 적용하는 AdaIN 실험을 진행했다.

### 실제로 수행한 실험

- 계절별 style 이미지들을 개별적으로 적용한 결과를 생성
- 같은 폴더의 style 이미지 5장을 interpolation 하여 계절별 대표 스타일을 생성
- 이 interpolation 스타일을 모든 개별 타일에 적용
- 결과를 스프라이트시트로 다시 합쳐 전체 타일셋 형태에서도 확인

### 관찰된 결과

결과 이미지는 `색감 변화`는 보이지만, 많은 경우 `오브젝트의 형태가 뚜렷하게 유지되지 않고 무엇인지 알아보기 어려운 수준`의 출력이 나타났다.

즉,

- 계절 분위기나 색감은 일부 반영됨
- 하지만 타일 안 오브젝트의 구조나 경계가 충분히 보존되지 않음

### 현재 세운 가설

이 현상의 원인으로 `모델이 각 오브젝트별 영역을 충분히 인식하지 못하고 전체 이미지를 하나의 질감처럼 처리했기 때문`이라는 가설을 세웠다.

특히 게임 타일은

- 크기가 작고
- 오브젝트 경계가 중요하며
- 같은 타일 안에서도 여러 구성 요소가 섞여 있기 때문에

일반 자연 이미지 기반의 AdaIN이 구조 보존에 불리할 수 있다고 판단했다.

### 이 가설을 검증하기 위한 다음 단계

이 가설을 입증하거나 반박하기 위해 아래 두 가지 방향으로 추가 실험을 진행할 예정이다.
...
이 조정을 통해 형태 보존과 스타일 강도의 균형이 좋아지는지 확인한다.

2. `mask 기능 추가`
- 타일 내부에서 영역을 나누어 서로 다른 스타일 적용 가능성 검토
- 특정 오브젝트/배경 영역을 분리할 수 있을 때 구조 보존이 나아지는지 확인

즉, 현재 논리는 다음과 같이 정리할 수 있다.

- 계절감을 잘 보여주는 style 이미지를 모아 interpolation 스타일을 만든다
- 이 스타일을 모든 타일에 적용해 계절별 자동 스타일 변환 가능성을 본다
- 결과가 구조적으로 불안정하게 나오면, 그 원인을 `영역 인식 부족`으로 가정한다
- 이를 검증하기 위해 `파라미터 튜닝`과 `mask 적용`을 다음 실험 단계로 설정한다
